...
`osu_dataset.npz`（学習データ）は第三者の譜面から作成しているため配布物には含めていない。
推論時に必要な「音符間隔バケットの境界」は、学習時の実測値 `[86.0, 120.0]` を
`chart_ai_osu.py` に固定値として持たせてあるため、**npz が無くても学習時と同じ分割で動作する**。
学習をやり直す場合は `docs/TRAINING.md` を参照（自分で用意した譜面から npz を再生成する）。
